...
jlong
Java_com_example_hellojni_HelloJni_encode(JNIEnv* env, jobject obj, 
jlong lfsr_state, jfloatArray x_java, jfloatArray y_java) {
	...
	unsigned int new_lfsr_state = encode(lfsr_state, N, K, x, y);
...
jlong
Java_com_example_hellojni_HelloJni_decode(JNIEnv* env, jobject obj,
jlong lfsr_state, jfloatArray y_java, jfloatArray xhat_java) {
	...
	int new_lfsr_state = decode(lfsr_state, K, N, y, xhat);
