...
int lpfilt( int datum ,int init)
	...
	static int* data = new int[LPBUFFER_LGTH] ;
  static int ptr = 0 ;
	long y0 ;
	int output, halfPtr ;
	if(init)
...
int hpfilt( int datum, int init )
	{
	static long y=0 ;
	static int* data = new int[HPBUFFER_LGTH];
  static int ptr = 0 ;
	int z, halfPtr ;
	if(init)
		{
...
int deriv1(int x, int init)
	{
	static int* derBuff = new int[DERIV_LENGTH] ;
  static int derI = 0 ;
	int y ;
	if(init != 0)
		{
...
int mvwint(int datum, int init)
	{
	static long sum = 0 ;
	static int* data = new int[WINDOW_WIDTH] ;
  static int ptr = 0 ;
	int output;
	if(init)
		{
