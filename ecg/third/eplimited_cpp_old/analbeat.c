...
*****************************************************************/
int IsoCheck(int *data, int isoLength, int isolimit) {
  int i, max, min ;
...
void AnalyzeBeat(int *beat, int *onset, int *offset, int *isoLevel,
  ...
  int slope, i;
	...
	for (i = FIDMARK-ISO_LENGTH2; i > 0; --i)
    if (IsoCheck(&beat[i],ISO_LENGTH2)) break;
  ...
  if (i == 0) {
	...
	for (i = FIDMARK; i < BEATLGTH; ++i)
    if (IsoCheck(&beat[i],ISO_LENGTH1) break;
	isoEnd = i;
  ...
  if (maxSlope > -minSlope)
    ...
    for (i = maxSlopeI; i > 0; --i)
      if ((beat[i]-beat[i-1]) <= (maxSlope >> 2)) break;
    *onset = i-1 ;

		// Check to see if this was just a brief inflection.
    for (; i > *onset-INF_CHK_N; --i)
      if ((beat[i]-beat[i-1]) > (maxSlope >>2)) break;
    ...
    for(;i > *onset-INF_CHK_N; --i)
      if ((beat[i-1]-beat[i]) >= (maxSlope>>2)) break;
    ...
    for(i = minSlopeI; i < BEATLGTH; ++i)
      if ((beat[i] - beat[i-1]) >= (minSlope >>2)) break;
		*offset = i;

		// Make sure this wasn't just an inflection.
		for(; i < *offset+INF_CHK_N; ++i)
      if ((beat[i]-beat[i-1]) < (minSlope>>2)) break;
    ...
    for (; i < *offset+BEAT_MS40; ++i)
      if ((beat[i-1]-beat[i]) <= (minSlope>>2)) break;
    ...
    if (i < *offset+BEAT_MS40) {
      ...
      for(; i < *offset+BEAT_MS60; ++i)
        if (beat[i]-beat[i-1] <= (minSlope>>2)) break;

      if(i < *offset + BEAT_MS60) {
        for(;i < BEATLGTH; ++i)
          if (beat[i]-beat[i-1] >= (minSlope>>2))
	  	*offset = i ;
      }
    }
  } else {
